...
% psf_path = 'C:\Research\RandoscopePSFS';
...
psf_path = '/home/rshuai/research/u-net-reconstruction/data/PSFs';
comps_path = [psf_path,'/SVD_2_5um_PSF_5um_1_ds4_dsz1_comps_green_SubAvg.mat'];
weights_path = [psf_path,'/SVD_2_5um_PSF_5um_1_ds4_dsz1_weights_green_SubAvg.mat'];
%%
%load data
% data = load('C:\Research\RandoscopePSFS\res_15_z30.mat');
% data = load('/home/rshuai/research/u-net-reconstruction/data/real-data/resTargetZ_1.mat')
data = load('/home/rshuai/research/u-net-reconstruction/results/best_images/sims/sim_676.mat')
data=data.b
b = data/max(data(:));
...
% 
%     params.data_format = file_info.FrameKey_0_0_0.x29981358_DataFormat;
%     params.bg_format = bg_info.FrameKey_0_0_0.x29981358_DataFormat;
...
% 
% 
% params.demosaic = 0;%contains(lower(params.data_format),'raw');
% params.demosaic_bg = 0;%contains(lower(params.bg_format),'raw');
...
%for zd = 9
    ...
    h = permute(h_in.array_out(:,:,1:params.rank,params.z_range),[1,2,4,3]);
    weights = permute(weights_in.array_out(:,:,1:params.rank,params.z_range),[1,2,4,3]);
...
%                 case('time')
%                     bg_raw = read_tiff_stack([bg_path,bg_name],1);
%                     if strcmpi(params.meas_slice,'all')
%                         data_raw = mean(double(read_tiff_stack(meas_path,1)),3);   %Average out the time variable
%                         if params.demosaic
%                             data_demos = imresize(double(demosaic(uint16(data_raw),'grbg')),params.ds_raw/params.ds,'box');
%                             data_demos=data_demos(:,:,2);
%                         else
...
%                             %grbg
%                         else
%                             data_demos = imresize(mean(data_in,3),params.ds_raw/params.ds,'box');
%                         end
%                     end
...
%                         bg_in=bg_in(:,:,2);
%                     else
%                         bg_in = imresize(mean(double(bg_raw),3),params.ds_bg/params.ds,'box');
%                     end
% 
%                     % data_raw = data_raw(:,:,:,1);
% 
...
%       
        ...
        options.convTol = 1e-11;
        
        %options.xsize = [256,256];
%         options.maxIter = 10000;
        options.maxIter = 5000;
        options.residTol = 5e-9;
        ...
        params.tau1 = 0.5e-7;%options.stepsize*.3e-3; %was 0.5e-7   %.000005 works pretty well for v1 camera, .0002 for v2
...
%         save_str = ['../results/best_images/recons/recon_569.mat'];
        save_str = ['../results/best_images/recons/recon_676_5000iters.mat'];
%         save_str = ['../results/best_images/recons/recon_res_target_5000iters.mat'];

%         full_path = fullfile(data_path,save_str);
        full_path = fullfile(save_str);
%         mkdir(full_path);
